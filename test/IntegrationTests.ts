...
describe("Integration - ConfigCatClient", () => {
    ...
    const clientOverride: IConfigCatClient = configcatClient.createClientWithAutoPoll(sdkKey, {
        flagOverrides: configcatClient.createFlagOverridesFromMap(
            { stringDefaultCat: "NOT_CAT" },
            configcatClient.OverrideBehaviour.LocalOnly,
        ),
    });
    ...
    it("getAllValuesAsync() should return all values", async () => {
        const sks: SettingKeyValue[] = await clientAutoPoll.getAllValuesAsync();
